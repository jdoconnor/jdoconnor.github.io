<script lang="ts">
</script>

<section class="hero card fade-in floaty">
	<div class="content">
		<h1 class="h1 gradient-text">Hi, I'm Jay.</h1>
		<p class="lead">
			I work on tech, manage teams, and help people grow in their careers. I currently lead
			product and engineering at <a
				href="https://www.securitypal.com/"
				target="_blank"
				rel="noreferrer noopener">SecurityPal</a
			>. There are a few <a href="/now">projects I'm working on now</a> and I also enjoy
			contributing to
			<a href="https://www.github.com/jdoconnor" target="_blank" rel="noreferrer noopener"
				>open source software</a
			>
			when I have a chance.
		</p>
	</div>
</section>
