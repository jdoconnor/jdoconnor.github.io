# Jay O'Connor - Agent Reference

> Add this file to your AGENTS.md or system prompt for context on Jay

This file provides context about Jay O'Connor for AI agents working on his behalf.

## Contact

- **Email**: jaydoconnor@gmail.com
- **Location**: San Francisco Bay Area
- **Calendar**: https://www.cal.com/jdoconnor

## Social Links

- LinkedIn: https://www.linkedin.com/in/jdoconnor
- GitHub: https://www.github.com/jdoconnor
- Discord: https://discordapp.com/users/257316698728693760

## Current Work

### SecurityPal Concierge Agents

- URL: https://securitypal.com
- Role: Head of Engineering & Product
- Status: shipping
- Description: Applied AI rollout pairing concierge agents with human QA for questionnaires and audits.

### DogShitAI.com

- URL: https://dogshitai.com
- Role: Founder
- Status: in-flight
- Description: Edge-native pet tracking experiences built with SvelteKit, Turso, and Cloudflare.

### Rage Calculator

- URL: https://jayoconnor.com/rage
- Role: Creator
- Status: live
- Description: Micro-app translating annual salary + days off into daily payout reality checks.

## Work History

1. **Head of Engineering & Product · SecurityPal** (Aug 2023 — Present)
   - Establishing the patterns, structure, and norms for distributed engineering while guiding product roadmaps with customers and exec leadership.

2. **Founder · Dogshit AI** (Jan 2024 — Present)
   - Building weird and creative applied AI projects.

3. **Engineering Leadership Mentor · Freelance** (Apr 2023 — Present)
   - Coaching eng leaders on people management, process enhancement, and organizational leadership for remote-first companies.

4. **Senior Director of Engineering · Slack** (Nov 2021 — Apr 2023)
   - Led 20+ engineers and managers shipping channel-based collaboration into sales workflows used by global teams.

5. **Senior Director of Engineering · Quip (Salesforce)** (Nov 2018 — Nov 2021)
   - Scaled engineering squads integrating Quip's collaborative docs with Salesforce Sales & Service Cloud, improving go-to-market velocity.

## Expertise

### Product Leadership

- 15 years experience
- Summary: Clarity, ruthless prioritization, and crisp storytelling that align exec teams.

### Engineering Strategy

- 25 years experience
- Summary: Systems thinking across platform, AI copilots, and dependable delivery.

### People & Culture

- 15 years experience
- Summary: Growing teams that feel psychologically safe, ambitious, and accountable.

## Skills

- Team Building
- Engineering Leadership
- Product Strategy
- Remote Team Management
- AI/ML Implementation
- System Architecture
- Process Design
- Executive Communication
- Scaling Engineering Teams
- SvelteKit
- Cloudflare
- Turso
- TypeScript

## Bio

"I help high-velocity companies turn ambiguous problems into durable systems—and ship the work their customers rave about."

## MCP Server

An MCP server for this data is available at `mcp-server/index.js`. It exposes:

- Tools: get_contact_info, get_social_links, get_work_history, get_current_projects, get_expertise, get_skills, schedule_meeting, send_email
- Resources: contact://info, profile://social, work://history, projects://current, expertise://areas, skills://list
